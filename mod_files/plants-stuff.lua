...
drawtype = "nodebox",
	...
	on_punch = function(pos, node, puncher)
			local tool = puncher:get_wielded_item():get_name()
			--minetest.chat_send_all("You hit a ".. node" with a :".. tool)
			if tool == "" or tool == "default:axe_wood" then
	...
	on_dig = function(pos, node, puncher)
			local tool = puncher:get_wielded_item():get_name()
			--minetest.chat_send_all("You hit a ".. node" with a :".. tool)
			if tool == "moreores:pick_mithril" then
				node.name = "beer_test:hops_9"
				minetest.env:set_node(pos, node)
				puncher:get_inventory():add_item("main", ItemStack("beer_test:hops"))
			else
				node.name = "air"
				minetest.env:set_node(pos, node)
				puncher:get_inventory():add_item("main", ItemStack("beer_test:growing_rope_down"))
			end
		end
})
